...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int n = io.ni(), d = io.ni();
		final var a = io.ni(n);
		final var lim = max(a) + 1;
		final var seg = new SegmentTree<Integer>(lim, Integer::max, 0);
		for(final int e: a) {
			final int l = max(0, e - d), r = min(lim - 1, e + d), m = seg.query(l, r + 1);
			seg.update(e, m + 1);
		}
		io.out(seg.all());
	}
