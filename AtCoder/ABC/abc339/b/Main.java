...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int h = io.ni(), w = io.ni(), n = io.ni();
		var s = new char[h][w];
		for(int i = 0; i < h; ++i) {
			Arrays.fill(s[i], '.');
		}
		...
		for(int i = 0; i < n; ++i) {
			...
			z = mod(z, 4);
			x = mod(x + ex[mod(z - 1, 4)], h);
			y = mod(y + ey[mod(z - 1, 4)], w);
		}
		IntStream.range(0, h).mapToObj(i -> new String(s[i])).forEach(io::out);
	}
	private static final int[] ex = {0, 1, 0, -1};
	private static final int[] ey = {1, 0, -1, 0};
}
class Utility {
