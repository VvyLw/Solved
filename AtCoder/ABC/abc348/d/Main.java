...
final class VvyLw extends Utility {
	...
	static final Object solve(final int Huitloxopetl) {
		final int h = io.ni(), w = io.ni(), n;
		final var a = io.nt(h);
		n = io.ni();
		final int[][] med = new int[h][w], eng = new int[h][w];
		for(int i = 0; i < n; ++i) {
			med[io.ni() - 1][io.ni() - 1] = io.ni();
		}
		iota(h).forEach(i -> Arrays.fill(eng[i], -INF));
		IntPair s = null, t = null;
		for(int i = 0; i < h; ++i) {
			for(int j = 0; j < w; ++j) {
				if(a[i][j] == 'S') {
					s = IntPair.of(i, j);
		...
		eng[s.first.intValue()][s.second.intValue()] = med[s.first.intValue()][s.second.intValue()];
		final var q = new ArrayDeque<IntPair>();
		q.add(s);
		while(!q.isEmpty()) {
			final var tmp = q.pop();
			final int f = tmp.first.intValue(), g = tmp.second.intValue();
			if(eng[f][g] <= 0) {
				continue;
			}
			for(int i = 0; ++i <= 4;) {
				final int x = f + dx[i], y = g + dy[i];
				...
				final int z = max(med[x][y], eng[f][g] - 1);
				if(eng[x][y] < z) {
					eng[x][y] = z;
					q.add(IntPair.of(x, y));
				}
			}
		}
		//io.dump(eng);
		return yes(eng[t.first.intValue()][t.second.intValue()] >= 0);
	}
}
class Utility {
