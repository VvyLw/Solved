...
void VvyLw::solve() {
	...
	auto dp=ndiv(n+1,p+1,p+1,p+1,p+1,p+1,1LL<<61);
	auto ad=[p](int x){ return min(p,x); };
	dp[0][0][0][0][0][0]=0;
	rep(n) {
		rep(j,p) rep(l,p) rep(m,p) rep(o,p) rep(q,p) chmin(dp[i+1][j][l][m][o][q],dp[i][j][l][m][o][q]);
		rep(j,p) rep(l,p) rep(m,p) rep(o,p) rep(q,p) {
			if(dp[i][j][l][m][o][q]==LINF) continue;
			chmin(dp[i+1][ad(j+a[i][0])][ad(l+a[i][1])][ad(m+a[i][2])][ad(o+a[i][3])][ad(q+a[i][4])],dp[i][j][l][m][o][q]+c[i]);
		}
	}
	auto res=dp[n][p][p][p][p][p];
	out(res==LINF?-1:res);
}