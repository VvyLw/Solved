...
final class VvyLw extends Utility {
	...
	static final void solve() {
		...
		final var s = sc.ns();
		final var a = new Data[n];
		for(int i = 0; i < n; ++i) {
			a[i] = new Data(s.charAt(i) - '0');
		}
		final var seg = new LazySegmentTree<Data, Integer>(a, Data::merge, (x, y) -> y == 1 ? x.flip() : x, (x, y) -> x ^ y, new Data(), 0);
		IntStream.range(0, q).forEach(i -> {
			final int t = sc.ni(), l = sc.ni() - 1, r = sc.ni();
			if(t == 1) {
				seg.apply(l, r, 1);
			} else {
				final var res = seg.query(l, r);
				o.out(yes(res.mx[0] <= 1 && res.mx[1] <= 1));
			}
			//dbg.out(seg);
		});
	}
	private static class Data {
		...
		static final Data merge(final Data l, final Data r) {
			final Data res = new Data();
			for(int i = 0; i < 2; ++i) {
				res.l[i] = l.l[i] + (l.l[i] == l.len ? r.l[i] : 0);
				res.r[i] = r.r[i] + (r.r[i] == r.len ? l.r[i] : 0);
				res.mx[i] = max(l.mx[i], r.mx[i], l.r[i] + r.l[i]);
			}
			res.len = l.len + r.len;
			return res;
		}
		@Override
		public final String toString(){ return "(" + mx[0] + ", " + mx[1] + ")"; }
	}
}
class Utility {
