...
final class VvyLw extends Utility {
	...
	static final void solve() {
		int h = io.ni(), w = io.ni(), m = io.ni();
		final var qry = io.ni(m, 3);
		final var clr = new long[200001];
		clr[0] = (long) h * w;
		final Set<Integer> sh = new HashSet<>(), sw = new HashSet<>();
		sh.add(0);
		sw.add(0);
		for(int i = m; --i >= 0;) {
			//io.dump(Arrays.copyOfRange(clr, 0, 6));
			final int t = qry[i][0], a = qry[i][1], x = qry[i][2];
			if(t == 1) {
				if(sw.contains(a)) {
					continue;
				}
		...
		io.out(Arrays.stream(clr).filter(i -> i > 0).count());
		iota(200001).filter(i -> clr[i] > 0).forEach(i -> io.out(i, clr[i]));
	}
}
class Utility {
