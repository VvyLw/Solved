...
void VvyLw::solve() {
	INT(n);
	STR(r,c);
	vi x=iot(n),y=iot(n),z=iot(n);
	const auto chk=[&](const vs &res) {
		unordered_set<char> us;
		rep(n) {
			rep(j,n) {
				if(res[i][j]=='.') continue;
				if(us.contains(res[i][j])) return false;
				us.emplace(res[i][j]);
			}
			if(ssize(us)!=3) return false;
			us.clear();
			rep(j,n) {
				if(res[j][i]=='.') continue;
				if(us.contains(res[j][i])) return false;
				us.emplace(res[j][i]);
			}
			if(ssize(us)!=3) return false;
			us.clear();
		...
		rep(n) {
			rep(j,n) {
				if(isalpha(res[i][j])) {
					if(res[i][j]!=r[i]) return false;
					break;
				}
			}
			rep(j,n) {
				if(isalpha(res[j][i])) {
					if(res[j][i]!=c[i]) return false;
					break;
				}
			}
		}
