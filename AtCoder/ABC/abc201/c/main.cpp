		...
		rep(j,10) {
			...
			rep(k,10) {
				...
				rep(l,10) {
					t+=l+'0';
					vi md;
					if(BS(pat,i)) md.emplace_back(i);
					if(BS(pat,j)) md.emplace_back(j);
					if(BS(pat,k)) md.emplace_back(k);
					if(BS(pat,l)) md.emplace_back(l);
					const bool ok=es[i]&&es[j]&&es[k]&&es[l];
					if(ok&&unq(md)==pat.size()) st.emplace(t);
					t.pop_back();
				}
				t.pop_back();
			}
