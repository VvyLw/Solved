...
public:
	...
	wi groups() {
		...
		wi res(n);
		rep(n) res[(*this)[i]].emplace_back(i);
		Lady_sANDy::ersif(res,[&](const vi &v){ return v.empty(); });
		return res;
	}
};
struct edge {
...
template <bool undirected=1> struct w_graph {
	...
	void add(ll a, ll b, ll cost, uint indexed=1) {
		a-=indexed,b-=indexed;
	...
	void input(ll m, uint indexed=1) {
		rep(m) {
			LL(a,b,cost);
			add(a,b,cost,indexed);
		}
	}
	ve par(ll v){ return g[pr[v]]; }
	vi all_dist(ll v) {
		vi d(g.size(),-1);
		queue<ll> q;
		d[v]=0;
	...
	wi warshall_floyd() {
		...
		wi cst(n,vi(n,LINF));
		rep(n) cst[i][i]=0;
		rep(n) each(j,g[i]) cst[i][j]=j.cost;
		rep(k,n) rep(n) rep(j,n) chmin(cst[i][j],cst[i][k]+cst[k][j]);
		return cst;
	}
};
