...
import math.big{integer_from_i64,integer_from_string}
...
pub fn solve()! {
	...
	mut x:=[2,1,0,1]
	mut y:=[1,2,1,0]
	ar.rotate_left(mut x,int(mod(a,4)))
	ar.rotate_left(mut y,int(mod(a,4)))
	if mod(b,2)==1 {
		x,y=y.clone(),x.clone()
	}
	// dbg(x)
	...
	mut s:=(w/4)*(h/2)*8
	for i in 0..w%4 {
		s+=x[i]*int(ma.ceil(f64(h)/2))
		s+=y[i]*int(ma.ceil(f64(h-1)/2))
		// dbg(s)
	}
	if h%2==1 {
		s+=(w/4)*4
