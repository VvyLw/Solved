...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int n = sc.ni(), m = sc.ni();
		final var uf = new WeightedUnionFind(n);
		for(int i = 0; i < m; ++i) {
			final int x = sc.ni() - 1, y = sc.ni() - 1, v = sc.ni();
			if(uf.unite(x, y, v) == -1 && abs(uf.diff(x, y)) != v) {
				o.fin("No");
			}
		}
		for(final var g: uf.groups()) {
			final var a = g.stream().mapToLong(i -> uf.get(i)).toArray();
			if(!scope(0, max(a) - min(a), (int) 1e9)) {
				o.fin("No");
			}
		}
		o.out("Yes");
