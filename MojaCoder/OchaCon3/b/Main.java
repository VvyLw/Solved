...
final class VvyLw extends Utility {
	...
	static final void solve() {
		...
		final int[] a = sc.ni(n), b = sc.ni(n);
		final IntPair[] p = new IntPair[n];
		for(int i = 0; i < n; ++i) {
			p[i] = IntPair.of(a[i], b[i]);
		}
		Arrays.sort(p, Comparator.<IntPair, Long>comparing(i -> i.first - i.second).thenComparing(i -> -i.second));
		//dbg.outl(p);
		int s = 0;
		final var dp = new long[n + 1];
		for(int i = 0; i < n; ++i) {
			final long x = dp[i] + p[i].first;
			if(dp[i + 1] < x) {
				dp[i + 1] = x;
			}
			final long y = dp[i] + p[i].second;
			if(s < k && dp[i + 1] < y) {
				dp[i + 1] = y;
				s++;
			}
