...
pub fn solve()! {
	n:=100000
	t:=[]u64{len:n,init:u64(index+1)}.map(it*(it+1)/2)
	p:=[]u64{len:n,init:u64(index+1)}.map(it*(3*it-1)/2)
	h:=[]u64{len:n,init:u64(index+1)}.map(it*(2*it-1))
	mut i,mut j,mut k:=285,165,143
	for {
		if t[i]==p[j] && p[j]==h[k] {
			println(t[i])
			return
