...
final class VvyLw extends Utility {
	...
	static final void solve() {
		int d = 1, sun = 0;
		final var sm = Arrays.asList(4, 6, 9, 11);
		for(int i = 1900; i <= 2000; ++i) {
			for(int j = 1; j <= 12; ++j) {
				final int m = sm.contains(j) ? 30 : j == 2 ? isLeap(i) ? 29 : 28 : 31;
				if(i != 1900 && d == 0) {
					sun++;
				}
				d = (int) mod(d + m, 7);
	...
	private static final boolean isLeap(final int y) {
		if(y % 4 == 0) {
			return y % 100 == 0 ? y % 400 == 0 : true;
		}
		return false;
	}
}
