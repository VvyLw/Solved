...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int n = 28123;
		final IntPredicate abundant = x -> div(x).stream().mapToLong(i -> i).sum() > 2 * x;
		...
		final var m = s.stream().max(Comparator.comparingInt(k -> k)).get();
		o.out(IntStream.range(1, n).filter(i -> !s.contains(i)).mapToLong(i -> i).sum());
	}
}
class Utility {
