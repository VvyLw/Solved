...
pub fn solve()! {
	...
	for k in 0..10 {
		for i in 100..1000 {
			...
			mut ok := true
			for _ in 0..3 {
				ok &&= p[s.join('').int() * 10 + k]
				ar.rotate_right(mut s, 1)
			}
			if ok {
				mut x:=[]int{}
				for _ in 0..3 {
...
fn f(x u64, st u64, n u64) u64 { return ((uint128_from_64(x)*uint128_from_64(x)+uint128_from_64(st))%uint128_from_64(n)).str().u64() }
fn g(n u64) u64 {
	if miller_rabin(n) {
		return n
	}
