...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int n = sc.ni();
		final var s = sorted(sc.ni(n));
		final int q = sc.ni();
		o.out(Arrays.stream(sc.ni(q)).filter(i -> binarySearch(s, i)).count());
	}
	private static final boolean binarySearch(final int[] a, final int x){ return Arrays.binarySearch(a, x) >= 0; }
}
class Utility {
