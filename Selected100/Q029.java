...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int r = io.ni(), c = io.ni();
		final IntPair s = io.pi().sub(IntPair.ONE), g = io.pi().sub(IntPair.ONE);
		final var t = io.nt(r);
		final BiFunction<IntPair, IntPair, Integer> bfs = (v1, v2) -> {
			final var q = new ArrayDeque<IntPair>();
			q.add(v1);
			final var d = new int[r][c];
			iota(r).forEach(i -> Arrays.fill(d[i], -1));
			d[v1.first.intValue()][v1.second.intValue()] = 0;
			while(!q.isEmpty()) {
				final var tmp = q.poll();
				for(int i = 0; ++i <= 4;) {
					final var mv = tmp.add(IntPair.of(dx[i], dy[i]));
					final int x = mv.first.intValue(), y = mv.second.intValue();
					if(!scope(0, x, r - 1) || !scope(0, y, c - 1) || t[x][y] == '#' || d[x][y] != -1) {
						continue;
					}
					d[x][y] = d[tmp.first.intValue()][tmp.second.intValue()] + 1;
					q.add(mv);
				}
			}
			return d[v2.first.intValue()][v2.second.intValue()];
		};
		io.out(bfs.apply(s, g));
	}
}
class Utility {
