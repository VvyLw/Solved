...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int n = sc.ni();
		final var p = sorted(sc.pi(n));
		long ans = 0;
		for(int i = 0; i < n; ++i) {
			for(int j = i; ++j < n;) {
				final int xi = p[i].first.intValue(), yi = p[i].second.intValue(), xj = p[j].first.intValue(), yj = p[j].second.intValue();
				final IntPair q = new IntPair(xj - yj + yi, yj + xj - xi), r = new IntPair(xi - yj + yi, yi + xj - xi);
				if(binarySearch(p, q) && binarySearch(p, r)) {
					ans = max(ans, sqr(xi - xj) + sqr(yi - yj));
				}
			}
		}
		o.out(ans);
