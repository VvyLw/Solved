...
final class VvyLw extends Utility {
	...
	static final void solve() {
		...
		final var q = sc.pi(n);
		for(int i = 0; i < n; ++i) {
			final var r = new IntPair(q[i].first.intValue() - p[0].first.intValue(), q[i].second.intValue() - p[0].second.intValue());
			final var s = Arrays.stream(p).map(k -> k.add(r)).toArray(IntPair[]::new);
			final var ok = new int[m];
			for(final var el: q) {
				for(int j = 0; j < m; ++j) {
					if(s[j].equals(el)) {
						ok[j] = 1;
			...
			if(Arrays.stream(ok).allMatch(k -> k == 1)) {
				o.fin(r);
			}
		}
	}
