...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int n = sc.ni();
		final var p = sc.pi(n);
		final LongPredicate fn = x -> {
			 if((Arrays.stream(p).mapToLong(i -> i.first).max().getAsLong()) > x) {
				 return false;
			 }
			 final var tmp = IntStream.range(0, n).mapToLong(i -> (x - p[i].first) / p[i].second).sorted().toArray();
			 boolean ok = true;
			 for(int i = 0; i < n; ++i) {
				 ok &= i <= tmp[i];
			 }
