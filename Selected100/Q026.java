...
final class VvyLw extends Utility {
	...
	static final void solve() {
		...
		final var g = new Graph(n, 1, true);
		g.input(n - 1);
		final var cnt = new int[n];
		for(int i = 0; i < q; ++i) {
			cnt[io.ni() - 1] += io.ni();
		}
		final IntFunction<int[]> bfs = v -> {
			final var vis = new boolean[n];
			final var que = new ArrayDeque<Integer>();
			vis[v] = true;
			que.add(v);
			while(!que.isEmpty()) {
				final int tmp = que.poll();
				for(final Edge el: g.get(tmp)) {
					if(vis[el.to]) {
						continue;
					}
					vis[el.to] = true;
