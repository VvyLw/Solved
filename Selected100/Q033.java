...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int h = io.ni(), w = io.ni();
		final var s = io.nt(h);
		int ans = iota(h).map(i -> (int) iota(w).filter(j -> s[i][j] == '.').count()).sum() - 1;
		final var d = new int[h][w];
		iota(h).forEach(i -> Arrays.fill(d[i], -1));
		d[0][0] = 0;
		...
		while(!q.isEmpty()) {
			...
			for(int i = 0; ++i <= 4;) {
				final int x = tmp.first.intValue() + dx[i], y = tmp.second.intValue() + dy[i];
				if(!scope(0, x, h - 1) || !scope(0, y, w - 1) || s[x][y] == '#' || d[x][y] != -1) {
					continue;
				}
		...
		//io.dump(ans, d[h - 1][w - 1]);
		//io.dump(d);
		final int x = d[h - 1][w - 1];
		if(x == -1) {
			io.fin(-1);
		}
		ans -= x;
