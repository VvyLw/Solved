...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int n = sc.ni();
		final var p = sc.pi(n);
		final Function<long[], Long> mean = a -> {
			final int m = a.length;
			return m % 2 != 0 ? sorted(a)[m / 2] : (sorted(a)[m / 2 - 1] + sorted(a)[m / 2]) / 2;
		};
		final long x = mean.apply(first(p)), y = mean.apply(second(p));
		long ans = 0;
		for(int i = 0; i < n; ++i) {
			ans += Math.abs(x - p[i].first);
			ans += Math.abs(p[i].first - p[i].second);
			ans += Math.abs(y - p[i].second);
		}
		o.out(ans);
	}
}
