...
final class VvyLw extends Utility {
	...
	static final Object solve(final int Huitloxopetl) {
		final int n = io.ni(), s = io.ni();
		final var a = io.ni(n);
		if(n == 1) {
			io.out(1);
			return 1;
		}
		final var p = iota(n).mapToObj(i -> IntPair.of(a[i], i + 1)).toArray(IntPair[]::new);
		Arrays.sort(p);
//		io.dump(p);
		...
		for(int i = 0; i < n; ++i) {
			if(i == 0) {
				...
				if(p[i + 1].first - p[i].first > s && p[i].first - p[i - 1].first > s) {
					res.add(p[i].second.intValue());
				}
			}
		}
