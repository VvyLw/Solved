...
final class VvyLw extends Utility {
	...
	static final void solve() {
		final int n = io.ni(), p = io.ni(), q = io.ni();
		final var a = io.ni(n);
		final QuadFunction<Integer, Integer, Integer, Integer, Integer> fn = (x, y, z, w) -> mod(intPow(10, x, p) + intPow(9, y, p) + intPow(7, z, p) + intPow(5, w, p), p);
		int cnt = 0;
		for(int i = 0; i < n; ++i) {
			for(int j = i; ++j < n;) {
				for(int k = j; ++k < n;) {
					for(int l = k; ++l < n;) {
						if(fn.apply(a[i], a[j], a[k], a[l]).equals(q)) {
							cnt++;
						}
					}
				}
