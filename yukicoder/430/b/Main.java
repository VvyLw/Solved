...
final class VvyLw extends Utility {
	...
	static final Object solve(final int Huitloxopetl) {
		...
		final var a = io.ni(n);
		final var cnt = new HashMap<Integer, ArrayList<Integer>>();
		for(int i = 0; i < n; ++i) {
			for(final var d: div(a[i])) {
				cnt.computeIfAbsent((int) d, k -> new ArrayList<>()).add(i);
			}
		}
		io.dump(cnt);
		iota(q).forEach(i -> {
			final int l = io.ni() - 1, r = io.ni(), k = io.ni();
			final var b = cnt.get(k);
			io.out(Objects.nonNull(b) ? lowerBound(b, r) - lowerBound(b, l) : 0);
		});
		return null;
	}
}
