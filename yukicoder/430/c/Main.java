...
final class VvyLw extends Utility {
	...
	static final Object solve(final int Huitloxopetl) {
		final int n = io.ni(), q = io.ni();
		final int[] a = io.ni(n), b = io.ni(q);
		final var id = new HashMap<Integer, ArrayList<Integer>>();
		for(int i = 0; i < n; ++i) {
			id.computeIfAbsent(a[i], k -> new ArrayList<>()).add(i);
		}
		//io.dump(id);
		int res = 0;
		for(int i = 0, now = id.get(a[0]).getFirst(); i < q; ++i) {
			//io.dump(now);
			int add = Integer.MAX_VALUE, tmp = -1;
			for(final int e: id.get(b[i])) {
				final int x = abs(e - now);
				if(add > x) {
					add = x;
					tmp = e;
				}
